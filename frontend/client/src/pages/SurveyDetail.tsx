...
export default function SurveyDetail() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (fee > 0) {
        ...
        // ★ 修正 Bug 2：betB 需要 msg.value > 0，entryFee=0 時直接報錯
        ...
        if (survey.poolType === "B") {
          if (!survey.contractPoolId) {
            // ★ 修正 Bug 2b：給出更清楚的錯誤說明
            toast.error("此問卷尚未綁定鏈上 Pool B ID", {
              description: "請等待合約交易確認，或由創建者在問卷詳情頁重新綁定 contractPoolId",
            });
            return;
          }
          const chainQ = getPoolBChainQuestion(survey);
          ...
          // ★ 修正：使用 ethers Interface 動態編碼 voteA，避免硬編碼 selector
          const ifaceVote = new ethers.Interface(["function voteA(uint256 _id)"]);
          calldata = ifaceVote.encodeFunctionData("voteA", [BigInt(survey.contractPoolId ?? surveyId)]);
          gasHex = "0x30000";
          toastMsg = "請在錢包確認 voteA（鏈上參與）交易…";
        }
  ...
  const handleFundContract = async () => {
    ...
    setIsFunding(true);
    try {
      // ★ 修正：新合約無 fundSurvey；直接以 ETH transfer 補存獎金
      const data = "0x";
  ...
  const handleDraw = async () => {
    ...
    try {
      ...
      if (poolType === "A") {
        ...
        // Pool B 抽獎由下方「步驟二」按鈕處理（resolveAndDrawB）
  ...
  const handleDrawFromQualified = async () => {
    ...
    try {
      ...
      if (poolType === "A") {
        // ★ 修正：Pool A 使用 ethers Interface 動態編碼 drawA
            ...
            <CardContent className="space-y-3">
              ...
              {survey.poolType === "B" && (revealResult || survey.qualifiedAddresses) && survey.status !== "drawn" && (
                <div className="space-y-3">
                  ...
                  {/* ★ 新增：Pool B resolveAndDrawB — 創建者選擇正確答案 */}
                  {survey.poolType === "B" && (
                    <div className="space-y-2">
                      <div className="flex items-center justify-between p-3 bg-white rounded-lg border border-orange-100">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            選擇 Pool B 題目的正確選項（0-based 索引），送給合約 resolveAndDrawB
                          </p>
                        </div>
                        <Button
                          size="sm"
                  ...
                  <div className="flex items-center justify-between p-3 bg-white rounded-lg border border-purple-100">
                    <div>
                      <p className="text-sm font-medium">
                        {survey.poolType === "B" ? "步驟二-B：觸發抽獎（resolveAndDrawB）" : "步驟二：觸發 Chainlink VRF 抽獎"}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {survey.poolType === "B"
                          ? `合約將篩選答對者並觸發 VRF 抽出 ${survey.winnerCount} 位中獎者`
                    ...
                    <Button
                      ...
                      disabled={
                        isDrawing ||
                        (survey.poolType !== "B" && (revealResult?.qualifiedCount ?? 0) === 0) ||
                        (survey.poolType === "B" && poolBCorrectAnswer === null)
                      }
                      className="gap-2 bg-purple-600 hover:bg-purple-700 text-white border-0"
                    >
                      <Shuffle className="w-4 h-4" />
            ...
            <CardContent className="space-y-6">
              ...
              <Button
                ...
                disabled={
                  isSubmitting ||
                  !isConnected ||
                  // ★ 修正 Bug 1：移除錯誤的 contractPoolId 封鎖條件
                  // contractPoolId 檢查已移至 handleSubmit 內部，在實際呼叫 betB 時才報錯
                  (parseFloat(survey.entryFee ?? "0") > 0 &&
