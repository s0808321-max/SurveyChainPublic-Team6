...
// GetWonSurveys GET /api/users/:address/won-surveys
// 回傳指定錢包地址曾中獎的問卷 ID 列表（供 Navbar 提領功能使用）
func GetWonSurveys(c *gin.Context) {
	wallet := strings.ToLower(c.Param("address"))
	if wallet == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "缺少錢包地址"})
		return
	}

	var participants []models.Participant
	if err := db.DB.
		Where("wallet_address = ? AND is_winner = ?", wallet, true).
		Find(&participants).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "查詢中獎紀錄失敗"})
		return
	}

	surveyIds := make([]uint, len(participants))
	for i, p := range participants {
		surveyIds[i] = p.SurveyID
	}
