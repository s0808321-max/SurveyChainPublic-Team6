	...
	func Register(r *gin.Engine) {
		...
		surveys := api.Group("/surveys")
		{
			...
			surveys.GET("/:id/qualified", handlers.GetQualifiedParticipants)
			...
			surveys.POST("/:id/answers", middleware.AuthRequired(), handlers.PublishAnswers)
		}

		// ─── 使用者路由 ───────────────────────────────────────────────────────────
		// ★ 新增：查詢指定錢包地址的中獎問卷 ID 列表（供 Navbar 提領按鈕使用）
		users := api.Group("/users")
		{
			users.GET("/:address/won-surveys", handlers.GetWonSurveys)
		}
	}
